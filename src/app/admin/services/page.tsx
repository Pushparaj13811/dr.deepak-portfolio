...
export default function ServicesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        setToast({ type: 'success', message: editing ? 'Service updated successfully!' : 'Service added successfully!' });
        await loadServices();
        handleCancel();
      } else {
        setToast({ type: 'error', message: result.error || 'Failed to save service' });
      }
    } catch {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setToast({ type: 'error', message: result.error || 'Failed to delete service' });
      }
    } catch {
      setToast({ type: 'error', message: 'Network error. Failed to delete service.' });
    }
  };
  ...
  return (
    <AdminPageLayout title="Manage Services" description="Add, edit, or remove services">
      {toast && (
        <Toast
          type={toast.type}
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
          <div className="p-6 space-y-4">
            ...
            <Input
              ...
              placeholder="e.g., medical, treatment, equipment"
            />
          </div>
