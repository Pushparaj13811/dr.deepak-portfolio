...
export function Hero({ profile, socialLinks }: HeroProps) {
  ...
  // Prioritize and limit social links
  const getPrioritizedLinks = () => {
    const priority = ['facebook', 'instagram', 'twitter', 'linkedin'];
    const prioritized: SocialLink[] = [];
    const others: SocialLink[] = [];

    socialLinks.forEach(link => {
      const platform = link.platform.toLowerCase();
      if (priority.includes(platform)) {
    ...
    prioritized.sort((a, b) => {
      const aIndex = priority.indexOf(a.platform.toLowerCase());
      const bIndex = priority.indexOf(b.platform.toLowerCase());
  ...
  const getSocialIcon = (platform: string) => {
    ...
    switch (normalizedPlatform) {
      ...
      default:
        return (
          <path d="M13.632 13.635h2.37V24H13.632v-10.365zm1.175-3.753c-.76 0-1.376-.616-1.376-1.376 0-.76.616-1.376 1.376-1.376.76 0 1.376.616 1.376 1.376 0 .76-.616 1.376-1.376 1.376zm-5.57 3.753h2.252V24H9.237v-10.365z" />
        );
    }
  };

  const displayedLinks = getPrioritizedLinks();
                ...
                <div className="flex gap-3 md:gap-4 pt-2 md:pt-4">
                  {displayedLinks.map((link) => (
                    <a
                      ...
                      className="w-10 h-10 flex items-center justify-center bg-white/90 rounded-full shadow-md hover:bg-white transition border-2 border-white"
